...
## 3. Chaîne de valeur : `Rules → Signals/Features → ML assistif → Opportunity → Actions`
...
| `ML assistif` | Calcule une propension commerciale avec un modèle logistique classique CPU-only et expose des contributions de features. Il ne remplace pas l’éligibilité par règle. | **IMPLÉMENTÉ / PROUVÉ** techniquement sur données synthétiques. Le mode autorisé pour la suite est `ML_SHADOW` tant que les portes d’acceptation ne sont pas franchies ([ml-acceptance.md](../ml-acceptance.md)). | Labels BOA matures, calibration, performance par segment, validation indépendante et valeur commerciale sont **NON IMPLÉMENTÉS**. Le ML ne doit donc pas être présenté comme validé BOA. |
| `Opportunity` | Fusionne les preuves déterministes, la propension éventuellement disponible, les garde-fous, la priorité, l’horizon, les produits potentiels et l’explication. | **IMPLÉMENTÉ / PROUVÉ** par `opportunity_api.py`, l’audit et les parcours E2E. Le fallback `RULES_ONLY` est explicite et n’utilise aucun score ancien ou inventé ([résilience ML](../../backend/src/boa_oi/resilience/ml_client.py)). | `HYBRID_CANDIDATE` est hors MVP. L’autorisation d’un reranking opérationnel après shadow est une **HYPOTHÈSE À VALIDER AVEC BOA** sur preuves métier et gouvernance. |
...
La chaîne ne doit pas être lue comme une autorité unique. `Analytics` reste propriétaire des métriques ; `Signal Service` et `Rule Engine` des sorties déterministes ; `Feature Store` des snapshots ; `ML Engine` du score contractuel ; `Opportunity Service` de la fusion, des garde-fous et de l’explication. Cette séparation est décrite dans [ml-engine.md](../ml-engine.md).
