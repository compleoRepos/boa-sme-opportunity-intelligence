...
Le ML est un **modèle classique CPU-only**, utilisé en `POC_ASSISTIVE` et à présenter en **POC/shadow** tant que des labels BOA matures, représentatifs et gouvernés ne sont pas disponibles. Le dépôt ne démontre pas un entraînement sur historique BOA, une calibration BOA ou une performance prédictive BOA. **Aucun LLM et aucun GPU ne sont requis** : l’inférence locale déterministe fonctionne sur CPU. Le LLM reste une extension architecturale future et n’est ni appelé ni requis par le runtime.
...
| **Preuve technique** | **IMPLÉMENTÉ / PROUVÉ :** `tests/e2e/rm-opportunity.spec.ts` vérifie la navigation vers `/clients/SME-…`, les blocs `#health` et `#why`, le changement de période, le drawer d’opportunité, l’onglet « Signaux & évidence » et le drawer de propension contenant « Aucune décision de crédit ». [`CustomerSheetPage.tsx`](../frontend/src/features/customer/CustomerSheetPage.tsx) montre que les graphiques et les données de fiche sont lus par hooks API ; le commentaire de code précise que la date de référence vient de la dernière observation analytique et non d’une date codée en dur. |
...
| **Réponse courte** | « Non. C’est une propension commerciale pour prioriser une candidate déjà éligible au contact. Il n’y a aucune décision de crédit. Les performances BOA et la calibration BOA ne sont pas démontrées. » |
| **Plan de repli** | Si aucune opportunité n’est ouverte pour la PME choisie, montrer la section « Aucune opportunité ouverte » et expliquer qu’une absence est un résultat explicable. Ouvrir ensuite une PME de la file qui possède une opportunité. Si le drawer échoue, conserver la fiche et citer la chaîne `signals → rules → ML → opportunity` documentée dans [`finalization-status-2026-09-19.md`](./finalization-status-2026-09-19.md). |
...
| **Phrase à dire** | « Le métier configure une règle sans code ; le workflow sépare l’auteur et l’approbateur. Le registre ML montre une version et sa lignée. Le ML complète la priorité commerciale, en CPU-only et en POC/shadow ; il ne décide jamais du crédit. Aucun LLM ni GPU n’est requis. » |
| **Preuve technique** | **IMPLÉMENTÉ / PROUVÉ :** `tests/e2e/rule-studio.spec.ts` couvre création visuelle, statut `Brouillon`, validation, simulation, soumission, séparation auteur/approbateur, approbation, publication, onglet « Versions & audit » et `Model Registry`. `tests/e2e/governance.spec.ts` vérifie une `Scoring Policy` active, `productionPerformanceClaim=false`, le mode `POC_ASSISTIVE`, les observations de monitoring, le RBAC administratif et le workflow MLOps gouverné. [`finalization-status-2026-09-19.md`](./finalization-status-2026-09-19.md) documente le fallback `RULES_ONLY`, la lignée, le registre et la readiness `BLOCKED`. **Limite :** les résultats de simulation, seuils et coefficients doivent être lus depuis l’API de la session ; ils ne doivent pas être inventés. |
...
| **Réponse courte** | « Nous montrons le socle de gouvernance et le chemin CPU du POC, pas une homologation. L’entraînement BOA, la calibration et les performances BOA sont **NON IMPLÉMENTÉS**. Le modèle reste `POC_ASSISTIVE`/shadow ; la readiness production est `BLOCKED`. » |
...
| ML | « Le modèle classique CPU-only est une aide de priorisation en `POC_ASSISTIVE`/shadow. » | « Le modèle est calibré, validé ou performant sur BOA ». |
