...
### 11.4 Model Registry, datasets et consentement

Une version de modèle conserve cible commerciale, horizon, population, feature set, dataset, cutoff, code, runtime, hyperparamètres, métriques globales/par segment, calibration, limites, artefact, checksum, SBOM et approbations. Les stages sont `DRAFT`, `VALIDATED`, `CHALLENGER`, `CHAMPION`, `RETIRED` et `REJECTED`. Il n’existe qu’un champion actif par cible, horizon et population. Auteur et approbateur sont distincts.

Un dataset d’apprentissage est immuable et manifesté. Il conserve finalité, label, horizon, population, période, cutoff, versions, hashes et approbations. Le consentement ou la base de traitement approuvée est vérifiable par sujet pseudonymisé et version de politique. `SIMULATED` et `NOT_REPORTED` sont exclus par défaut ; seuls les outcomes `OBSERVED` et matures sont éligibles. L’absence d’outcome n’est pas un label négatif.

### 11.5 Inférence, explication, fusion et drift

`propensity_scores` conserve client, type d’opportunité, outcome, horizon, score, interprétation, modèle, snapshot, qualité, validité et version de contrat. Le score est dans `[0,1]` uniquement si la qualité est `VALID` ; aucune valeur par défaut n’est autorisée. `prediction_explanations` conserve méthode, baseline, contributeurs, valeurs manquantes et avertissements.

`opportunity_fusion_decisions` conserve séparément règle, propension, politique de fusion, garde-fous, fallback et rang final. Les champs ou alias `credit_score`, `risk_score`, `default_probability`, `credit_decision` et équivalents sont interdits.
