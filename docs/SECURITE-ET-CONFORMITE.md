...
2. Le POC utilise un **ML classique CPU-only**, verrouillé en `POC_SHADOW`, avec priorité opérationnelle `RULES_ONLY`, faute de labels historiques BOA matures. **Aucun LLM ni GPU n’est requis, appelé ou autorisé dans le chemin actuel.** Les outcomes locaux restent des labels candidats ; aucune performance BOA ni calibration validée n’est revendiquée.
...
| ML et IA générative | **PROUVÉ** : modèle logistique CPU, `POC_SHADOW`, priorité `RULES_ONLY`, manifest/évaluation bloqués, absence de dépendance LLM/GPU. | Historiques BOA, calibration et performance réelle non démontrés ; toute influence ML est interdite. | **À VALIDER** : critères de sortie, données BOA et revue indépendante. |
...
Le score ML est une propension commerciale relative, non une probabilité de crédit. Le premier incrément reste sur données locales/synthétiques, sans historiques BOA matures, en `POC_SHADOW`; la priorité est `RULES_ONLY`. Les métriques Brier/ECE éventuellement calculées sont descriptives et la calibration reste `NOT_VALIDATED`. Aucune précision, AUC, lift, uplift, matrice de confusion ou KPI commercial BOA ne doit être fabriqué.
