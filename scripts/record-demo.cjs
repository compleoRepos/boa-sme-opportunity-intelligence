/* Enregistrement vidéo optionnel de la démonstration locale BOA SME Opportunity Intelligence.
 * Navigation réelle dans la stack locale, sur le jeu de données synthétique du dépôt.
 ...
 * Usage : node scripts/record-demo.cjs [acte1|acte4|acte5|acte6]
 * Ce script et ses sorties ne constituent ni une mesure de production ni une preuve AWS.
 */
...
async function acte1_2_3(browser) {
  ...
  await say(page, 'Chaque ligne est ordonnée par les règles métier. Signaux chiffrés, propension shadow et produit potentiel restent explicables avant le clic.', 1000)
  ...
  await say(page, 'Pourquoi cette opportunité ? Signaux détectés, règle publiée, opportunité rules-only et observation ML shadow sont séparés et tracés.', 3600)
  ...
  await say(page, 'Le score ML est observé en POC shadow : il ne modifie ni le classement rules-only ni une décision de crédit.', 2800)
...
async function acte6_7(browser, ruleUrl) {
  ...
  await say(page, 'Le modèle de propension POC shadow est gouverné : registre, versions, features, provenance et coefficients.', 800)
  ...
  await say(page, 'Model Registry : version shadow, bande descriptive et coefficients lisibles. Le poids ML opérationnel reste nul.', 1600)
  ...
  await pause(1500)
  await say(page, 'Stack locale intégrée et données synthétiques : démonstration technique, sans homologation de production ni performance ML revendiquée.', 3200)
  await say(page, '', 300)
