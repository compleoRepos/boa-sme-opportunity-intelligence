    ...
    for match in matches:
        evidence = tuple(
            ConditionEvidence(
                ...
                label=f"{item['metric']} satisfies the published Rule Studio condition",
            )
        ...
        candidates.append(
            OpportunityCandidate(
                ...
                what=f"Opportunity generated by published rule {rule_id} v{rule_version}",
                when=str(match.get("horizon") or "1-3_MONTHS"),
...
def rerank_with_propensity(
    ...
    *,
    ...
    combined = (
        (ml_weight * ml_score + rules_weight * rules_score) / denominator
        if denominator > 0
        else rules_score
    )
    priority_level: Literal["P1", "P2", "P3", "P4"] = (
    ...
    ml_component = {
        ...
        "weighted_value": ml_weight * ml_score,
        "weight": ml_weight,
...
async def context_for(
    ...
    product_status = {item["product"]["productId"]: item["status"] for item in gaps["gaps"]}
