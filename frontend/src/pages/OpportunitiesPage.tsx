  ...
  return <>
    ...
    <Panel flush id="opportunities">
      <form className="panel-body filter-row" onSubmit={submit}>
        ...
        <label className="field">Statut<select className="select sm" value={draft.status} onChange={(event) => set('status', event.target.value)}><option value="">Tous</option>{['OPEN', 'ACCEPTED', 'CONTACTED', 'DISMISSED', 'CONVERTED'].map((value) => <option key={value} value={value}>{label(value)}</option>)}</select></label>
