...
export function ProductsPage() {
  ...
  return <>
    <header className="page-head"><div><p className="eyebrow accent">Catalogue commercial</p><h1>Produits BANK OF AFRICA pour les entreprises</h1><p className="subtitle">Offre entreprises et PME relevée sur les pages publiques de bankofafrica.ma, servie par le Product Service. Le moteur raisonne par famille (lacune produit) ; les règles recommandent des produits précis.</p></div><div className="page-actions"><Segmented value={active} onChange={(value) => setParams(value ? { active: value } : {})} options={[{ value: 'true', label: 'Actifs' }, { value: 'false', label: 'Inactifs' }, { value: '', label: 'Tous' }]} /></div></header>
    {query.isPending ? <SkeletonStack rows={3} kind="block" /> : query.isError ? <ErrorState error={query.error} onRetry={() => void query.refetch()} /> : !query.data.data.length ? <EmptyState title="Aucun produit" /> : groups.map(([family, products]) => <section key={family} className="stack" style={{ gap: 12 }}>
      ...
      <div className="grid cols-3">{products.map((product) => <Panel key={product.productId} eyebrow={label(product.category)} title={<span className="row" style={{ gap: 8 }}><Landmark size={16} />{product.name}</span>} id={`product-${product.productId}`} tools={<Badge value={product.active ? 'ACTIVE' : 'INACTIVE'} />}>
        ...
        <dl className="kv"><dt>Identifiant</dt><dd className="mono">{product.productId}</dd><dt>Segments</dt><dd>{Array.isArray(product.targetSegment) ? product.targetSegment.map(label).join(', ') : label(product.targetSegment)}</dd><dt>Devises</dt><dd>{Array.isArray(product.currency) ? product.currency.join(', ') : product.currency || '—'}</dd></dl>
        <div className="row" style={{ gap: 12, marginTop: 10, alignItems: 'center' }}>
          ...
          {product.eligibilityRules && Object.keys(product.eligibilityRules).length > 0 && <details><summary className="muted" style={{ cursor: 'pointer', fontSize: 12 }}>Critères connus</summary><pre className="mono" style={{ whiteSpace: 'pre-wrap', fontSize: 11 }}>{safeJson(product.eligibilityRules)}</pre></details>}
        </div>
      </Panel>)}</div>
    </section>)}
  </>
